...
s3load("bronchStop2.Rdata", bucket = SECRET_BUCKETNAME)
...
recruitment_by_site_table|>
  slice_head(n=10)|>
  mutate(Rank=c(1:n()))|>
  select(Rank,sitename_short,total)|>
  gt()|>
  cols_align(align = "center",columns = total)|>
    cols_label(
    sitename_short = "Site Name",
    total = "Total")|>
  tab_header(
...
```{r}
recruitment_by_site_table|>
  arrange(sitename_short)|>
  select(sitename_short,total)|>
  gt()|>
  cols_align(align = "center",columns = total)|>
    cols_label(
    sitename_short = "Site Name",
...
map_data%>%
  ...
  #geom_point(  aes(x=long, y=lat, size=0.1, alpha=90), shape = 20) +
...
BronchStop is an extension of the [BronchStart study](https://doi.org/10.1093/infdis/jiad551), which initially examined the burden of respiratory disease in children under 2 years in the United Kingdom (UK) and Ireland. Findings from this study provided an estimate of the burden of RSV in the UK and played a role in the JCVI’s recommendation to introduce the maternal RSV vaccination. During the 2024-2025 season, BronchStop successfully evaluated vaccine effectiveness (VE) for the newly introduced maternal RSV vaccine in England and Scotland, and surveyed mothers to understand their views on the vaccine.  

For the 2025–2026 season, we are recruiting infants under six months old admitted with respiratory illness and determining whether their mothers were vaccinated against RSV. The protocol for the 2025-2026 season is available [here](https://openresearch.nihr.ac.uk/articles/5-85/v1). 
...
2) Our vaccine effectiveness (VE) results from the 2024-2025 season: the first real-world assessment of maternal RSV VE from a high-income country: [Bivalent prefusion F vaccination in pregnancy and respiratory syncytial virus hospitalisation in infants in the UK: results of a multicentre, test-negative, case-control study- The Lancet Child and Adolescent Health](https://www.thelancet.com/journals/lanchi/article/PIIS2352-4642(25)00155-5/fulltext).  
...
5) World Health Organization position paper on RSV immunisation [citing the BronchStop study](https://www.who.int/publications/i/item/WER10022).

6) [Transcript of a House of Lords debate citing the results from the BronchStop study](https://hansard.parliament.uk/lords/2025-09-09/debates/0AD30E5C-A3AA-40A4-A743-F7B655F06EB3/RespiratorySyncytialVirusVaccinationProgramme).  

7) [News piece in The Sun newspaper on the results from the BronchStop study](https://www.thesun.co.uk/health/35915701/cases-deadly-lung-virus-rsv-babies-plummets-vaccine/).
...
#ridge_graph
left_join(data.frame(studyWeek=c(0:24)),cumulative_viruses)%>%
